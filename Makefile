...
setup-backend:
	python -m pip install -e ./packages/contracts -e ./packages/classification -e ./packages/policy -e "./apps/api[dev]"
...
format-backend:
	python -m ruff format packages/contracts packages/classification packages/policy apps/api
